...
#table(
  ...
  [Substrate], [3D-printed PLA (requires consistent Z-axis pressure).],
  [Trajectory], [`movep` with blend radius $r$ for smooth curves.],
  [Gripper Force], [Regulated via `rFR` to protect the pen casing.],
  [Calibration], [Wrist Camera `Cam Locate` to find the duck's base support.],
  [Interface], [Socket communication on port 63352 for gripper control.],
)

= Code Implementation Logic
The final script utilizes the `URBasic` library to send URScript strings. The logic follows a sequence of:
...
5. Execute the drawing path using a series of `movep` waypoints.
